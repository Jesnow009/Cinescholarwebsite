...
for (let i = 0; i < lines.length; i++) {
    ...
    if (!line.includes(':') && currentDirector) {
        ...
        currentMovieObj = {
            ...
            composer: 'NA',
            studio: 'NA',
            director: 'NA', // some entries have this
            country: 'NA'
        };
        continue;
    }
...
async function updateData() {
    ...
    for (const dName of Object.keys(moviesByDirector)) {
        ...
        if (dirObj) {
            ...
            // clear existing mustWatch
            dirObj.mustWatch = [];
            
            for (let mv of requestedMovies) {
                // Fetch poster
                let poster = await scrapeTMDB(mv.title);
                if (!poster) {
                    poster = "assets/images/placeholder.png"; // Fallback to avoid NA
                }
                ...
                let m = {
                    ...
                    director: mv.director !== 'NA' ? mv.director : undefined,
                    writer: mv.writer,
                    cinematographer: mv.cinematographer,
                    editor: mv.editor,
                    composer: mv.composer,
                    studio: mv.studio,
                    poster: poster
                };
                
                // Keep old focus, plot if they existed for this movie
                // We'll just leave them out or as empty if not provided, 
                // wait, let's copy from original if it existed
                // Actually the user said "Remove the exisiting movies add below given movies"
                // Let's just add what's provided
                dirObj.mustWatch.push(m);
            }
        }
    }

    // Now stringify and save back
    let newContent = JSON.stringify(context.FILMS_DATA, null, 4);
    // Format appropriately back to js file
