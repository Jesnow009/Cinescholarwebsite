...
async function run() {
    ...
    for (const p of data.cinematographer.cinematographers) {
        // If the region has 'french' in its name
        if (p.region && p.region.includes('french')) {
            for (const m of p.mustWatch) {
                if (!m.poster || m.poster.includes('placeholder.jpg') || m.poster === '') {
                    ...
                    // Sometimes title has Original Title in it or English (French). Let's extract the first part.
