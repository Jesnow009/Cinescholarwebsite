...
async function processTamil() {
    ...
    // Assign back to FILMS_DATA safely!
    let updatedCount = 0;
    FILMS_DATA.director.directors.forEach(d => {
        if (directors[d.name]) {
            d.mustWatch = directors[d.name];
            updatedCount++;
        }
    });
    
    console.log(`Updated mustWatch for ${updatedCount} directors in FILMS_DATA object.`);
    
    const outputJs = `const FILMS_DATA = ${JSON.stringify(FILMS_DATA, null, 4)};\n\nexport default FILMS_DATA;`;
    fs.writeFileSync('js/data.js', outputJs, 'utf8');
    
    console.log('Successfully wrote data.js cleanly with robust JSON serialization!');
}
