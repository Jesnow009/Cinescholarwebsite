...
async function run() {
    ...
    for (const p of data.cinematographer.cinematographers) {
        // If the region has 'eastern' or 'european' in its name
        if (p.region && (p.region.includes('eastern') || p.region.includes('russian'))) { // including russian just in case they overlap
            for (const m of p.mustWatch) {
                if (!m.poster || m.poster.includes('placeholder.jpg') || m.poster === '') {
                    ...
                    // Sometimes title has Original Title in it or English (German). Let's extract the first part.
