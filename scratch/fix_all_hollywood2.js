...
async function run() {
    ...
    for (const p of data.cinematographer.cinematographers) {
        // If the region has 'hollywood' in its name, or 'north'
        if (p.region && (p.region.includes('hollywood') || p.region.includes('north'))) {
            for (const m of p.mustWatch) {
                if (!m.poster || m.poster.includes('placeholder.jpg') || m.poster === '') {
                    // Fetch poster
                    const query = encodeURIComponent(m.title.replace(/\(.*\)/, '').trim());
