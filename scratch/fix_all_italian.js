...
async function run() {
    ...
    for (const p of data.cinematographer.cinematographers) {
        // If the region has 'italian' in its name
        if (p.region && p.region.includes('italian')) {
            for (const m of p.mustWatch) {
                if (!m.poster || m.poster.includes('placeholder.jpg') || m.poster === '') {
                    ...
                    // Sometimes title has Original Title in it or English (Italian). Let's extract the first part.
