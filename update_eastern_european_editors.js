...
const focusMap = {
    "Ida": "Stark, minimalist editing perfectly timed to austere, static framing.",
    "Cold War": "Elliptical, fast-paced editing that condenses decades of turbulent romance into poetic fragments.",
    "Quo Vadis, Aida?": "Urgent, relentless pacing that heightens the impending sense of tragedy and panic.",
    "The Turin Horse": "Hypnotic, extremely slow pacing composed of long, continuous takes.",
    "Werckmeister Harmonies": "Mesmerizing, unbroken sequences emphasizing oppressive atmosphere and existential dread.",
    "Sátántangó": "Monumental, glacial pacing that completely reconstructs cinematic time.",
    "The Childhood of a Leader": "Ominous, dread-inducing transitions cut aggressively to an overwhelming orchestral score.",
    "White God": "Chaotic, kinetic action editing grounded by visceral realism and animal choreography.",
    "Pieces of a Woman": "Intense, fluid long-takes interwoven with sharp, jarring temporal jumps.",
    "EO": "Subjective, expressionistic editing capturing the disjointed, terrifying world from an animal's perspective.",
    "Lamb": "Deliberately slow, atmospheric pacing that slowly unravels creeping folk-horror.",
    "The Girl with the Needle": "Grim, meticulously paced editing that heightens psychological tension and period-piece claustrophobia."
};
...
async function updateData() {
    ...
    for (const dName of Object.keys(moviesByDirector)) {
        let dirObj = editors.find(d => d.name === dName && d.region === "eastern-european");
        if (!dirObj) dirObj = editors.find(d => d.name.includes(dName.split(' ')[1]) && d.region === "eastern-european");
        if (!dirObj) dirObj = editors.find(d => d.name.includes(dName.split(' ').pop()) && d.region === "eastern-european");
    ...
    console.log("Updated js/data.js for Eastern European Editors successfully.");
}
